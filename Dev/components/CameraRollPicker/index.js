...
import { SafeAreaView, View, Platform, FlatList, Animated, Pressable, Image, Modal, Button } from 'react-native';
...
class CameraRollPicker extends React.Component{
    ...
    onRefresh = () => {
        this.setState({media: this.defaultMedia, selected: [], after: undefined, hasNextPage: true}, () => {
    ...
    renderPicture = ({item, index, separators}) => {
        if(item == ':camera'){
            return(
                <Pressable
                    key={':camera'}
